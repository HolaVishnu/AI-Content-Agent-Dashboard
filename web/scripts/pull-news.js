...
function decodeEntities(s) {
  return (s || '')
    ...
    .replace(/&#8220;/g, 'â€œ').replace(/&#8221;/g, 'â€')
    .replace(/&#8211;/g, 'â€“').replace(/&#8230;/g, 'â€¦')
    .replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&apos;/g, "'")
    .replace(/&#x2019;/gi, 'â€™').replace(/&#x2018;/gi, 'â€˜')
    .replace(/&#x201c;/gi, 'â€œ').replace(/&#x201d;/gi, 'â€')
    .trim();
}
...
async function fetchFeed({ url, category, max }) {
  try {
    ...
    console.log(`  ${category} (${url}): FAILED â€” ${e.message}`);
    return [];
  }
}
...
async function main() {
  ...
  // Four-column grouping for the Dashboard tab â€” Space merges the four space-ish
  ...
  console.log(`\nâœ“ Saved ${top10.length} headlines to dashboard/news.json`);
}
