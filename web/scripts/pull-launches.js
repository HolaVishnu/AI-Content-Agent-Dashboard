...
// Pulls REAL upcoming rocket launches â€” every agency (SpaceX, ISRO, Roscosmos,
// ESA, NASA, CNSA, etc.), not just NASA â€” from the Launch Library 2 API
...
async function main() {
  ...
  const launches = (data.results || []).map(l => ({
    ...
    net: l.net, // "no earlier than" â€” the scheduled launch time, ISO
  ...
  const outPath = path.join(__dirname, '..', 'public', 'launches.json');
  fs.writeFileSync(outPath, JSON.stringify(output, null, 2));
  console.log(`\nâœ“ Saved ${launches.length} upcoming launches to dashboard/launches.json`);
  launches.slice(0, 3).forEach(l => console.log(`  - ${l.name} (${l.provider}) â€” ${l.net}`));
}
