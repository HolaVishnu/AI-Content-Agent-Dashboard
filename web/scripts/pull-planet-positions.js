...
// NASA JPL's Horizons API (no API key required â€” public JPL/SSD service) and
...
async function main() {
  ...
  for (const body of BODIES) {
    try {
      ...
      console.log(`  ${body.key}: angle=${pos.angleDeg.toFixed(2)}Â° distance=${pos.distanceAU.toFixed(3)} AU`);
    } catch (e) {
      console.log(`  ${body.key}: FAILED â€” ${e.message}`);
    }
  }

  const output = { pulledAt: now.toISOString(), source: 'NASA JPL Horizons (ssd.jpl.nasa.gov)', positions };
  const outPath = path.join(__dirname, '..', 'public', 'planet-positions.json');
  fs.writeFileSync(outPath, JSON.stringify(output, null, 2));
  console.log(`\nâœ“ Saved ${Object.keys(positions).length}/8 real planet positions to dashboard/planet-positions.json`);
}
