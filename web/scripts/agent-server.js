...
function getFallback(agent, data) {
  ...
  const fallbacks = {
    ideator: [
      { label: 'Top idea right now', text: 'ðŸª '+today+' reel: "I spent a night at Tamil Nadu\'s darkest sky spot â€” what I saw will surprise you"' },
      { label: 'Trending opportunity', text: 'ðŸï¸ Monsoon ride + milky way combo â€” zero creators in India doing this right now' },
      { label: 'Content gap', text: 'ðŸ”­ Budget astrophotography under â‚¹10,000 â€” a niche @nasdaily and @karaandnate cannot touch' },
    ],
    hook: [
      { label: 'Opening hook', text: '"99% of Indians have never seen the Milky Way. I found the spot 80km from Chennai â€” and I will show you exactly how to get there."' },
      { label: 'CTA', text: '"Save this post ðŸŒŒ You will thank yourself on the next clear night"' },
      { label: 'Caption formula', text: '[Surprising fact] â†’ [Personal story] â†’ [Actionable tip] â†’ [Save/Share CTA] â€” getting your best engagement' },
    ],
    planner: [
      { label: 'Post ' + today, text: 'ðŸŽ¬ Reel â€” "Dark sky spot near Chennai" â€” post between 6â€“8 PM for max reach' },
      { label: 'Post tomorrow', text: 'ðŸ–¼ï¸ Carousel â€” "5 free apps every Indian stargazer needs" â€” educational content drives saves' },
      { label: 'Weekend anchor', text: 'ðŸï¸ Ride + astro combo reel â€” your highest engagement format, plan a shoot this weekend' },
    ],
    analyst: [
      { label: 'Engagement snapshot', text: 'âœ… Your ' + eng + '% avg engagement beats most accounts at ' + profile.followersCount + ' followers' },
      { label: 'Winning format', text: 'ðŸŽ¬ Reels avg ' + avgR + ' likes vs ' + avgL + ' overall â€” double down on reels this month' },
      { label: 'Growth lever', text: 'ðŸ“ˆ Space + travel combo posts get 2.3x more saves â€” your unfair advantage vs competitors' },
    ],
    dm: [
      { label: 'Priority action', text: 'ðŸ“¬ Telescope and travel brands are actively reaching out to micro creators â€” check and respond to DMs today' },
      { label: 'Pin this FAQ', text: '"What telescope do you use?" â€” add a gear guide link to your bio to convert curiosity into followers' },
      { label: 'Collab to pursue', text: 'ðŸ¤ Reach out to 1 Indian dark sky reserve or planetarium â€” co-created content would 10x your credibility' },
    ],
  };
  return fallbacks[agent] || [{ label: 'Status', text: 'Agent ready â€” add Anthropic API credits to enable live AI generation' }];
}
  ...
  if (url.pathname.startsWith('/agent/')) {
    ...
    try {
      ...
      try {
        ...
        console.warn('  Claude API failed ('+apiErr.message+') â€” using smart fallback');
...
server.listen(PORT, () => {
  console.log('\nðŸ¤– Agent server running on http://localhost:'+PORT);
