...
async function main() {
  ...
  } catch (e) { console.log('  Solar flares: FAILED â€” ' + e.message); }
  ...
  try {
    ...
    cmes.forEach(c => events.push({
      ...
      icon: 'ðŸŒŠ',
      time: c.startTime,
      detail: c.note ? c.note.slice(0, 160) : 'CME detected â€” may affect Earth-facing space weather.',
      link: c.link,
    }));
    console.log(`  CMEs: ${cmes.length}`);
  } catch (e) { console.log('  CMEs: FAILED â€” ' + e.message); }
  ...
  } catch (e) { console.log('  Geomagnetic storms: FAILED â€” ' + e.message); }
  ...
  const outPath = path.join(__dirname, '..', 'public', 'spaceweather.json');
  fs.writeFileSync(outPath, JSON.stringify(output, null, 2));
  console.log(`\nâœ“ Saved ${output.events.length} space weather events to dashboard/spaceweather.json`);
}
